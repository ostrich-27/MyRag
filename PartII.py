...
class Model_Similarity(object):
    ...
    def __transform(self,df,timestamp):
        ...
        for code,details in df_Group.items():
            ...
            if str(result_sht.Cells(current_row, 3).Value).find("同类业务采用不同经营模式涉及不同收入确认方式及计量方法")>-1:
                result_sht.Cells(current_row, 19).Value="适用"
            else:
                result_sht.Cells(current_row, 19).Value="未在摘取的收入政策中找到"
            current_row += 1
            t2=time.time()
